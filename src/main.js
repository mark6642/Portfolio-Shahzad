...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 7. Contact Modal Overlay Handler
  ...
  if (contactModal && openModalBtn && closeModalBtn) {
    ...
    const closeModal = () => {
      ...
      if (modalForm) {
        modalForm.reset();
        const groups = modalForm.querySelectorAll('.form-group');
  ...
  // 8. Modal Contact Form Submission Handler
  if (modalForm && modalFormStatus) {
    modalForm.addEventListener('submit', (e) => {
      ...
      if (!isValid) {
        modalFormStatus.textContent = 'Please correct the errors in the form.';
